...
HTML не является языком программирования; это язык разметки, и используется, чтобы сообщать браузеру, как отображать веб-страницы.`,
  ...
  [
    ...
    [
      `
&lt!DOCTYPE html&gt используется для указания типа документа.
...
&lt!-- корневая обертка страницы --&gt
&lthtml lang=ru&gt
  &lt!-- тег который содержит все вспомогательные данные о HTML документе:
       - заголовок, описание, SEO-информация, подключение стилей и шрифтов, скриптов, meta-информация.
       Данные указанные внутри этого тега не отрисовываются на странице--&gt
  &lthead&gt
    &ltmeta charset=UTF-8 /&gt
    &lttitle&gtМоя первая страница&lt/title&gt
  &lt/head&gt

  &lt!-- тег, который содержит всю разметку HTML документа. 
       Именно эта разметка будет отображаться в браузере --&gt
...
&lttime&gt`,
  ...
  [
    [`5. Что описывается в тэге &lthead&gt?`],
    [
      `&lthead&gt
  &lt!-- Метаданные — данные, которые описывают данные.
  Многие типы &ltmeta&gt больше не используются. Так, поисковые системы больше не используют данные из элемента &ltmeta type=keywords content=ваши, ключевые, слова, введите, здесь&gt
   --&gt
  &lt!-- кодировка документа --&gt
  &ltmeta charset=UTF-8 /&gt
  &lt!-- Основной заголовок HTML-страницы --&gt
  &lttitle&gtМоя первая страница&lt/title&gt
  &lt!-- указание автора страницы  --&gt
  &ltmeta name=author content=Крис Миллс /&gt
  &lt!-- краткое описание её содержимого  --&gt
  &ltmeta
    name=description
    content=Задача MDN — в том, чтобы обучить
новичков всему тому, что нужно им для разработки веб-сайтов и приложений.
  /&gt
  &lt!-- Подключение иконок --&gt
  &ltlink
    rel=shortcut icon
    href=https://developer.mozilla.org/static/img/favicon32.png
  /&gt
  &lt!-- Подключение CSS --&gt
  &ltlink rel=stylesheet href=my-css-file.css /&gt
  &lt!-- Подключение JavaScript --&gt
  &ltscript src=my-js-file.js&gt&lt/script&gt
  &lt!-- Meta-тег viewport сообщает браузеру о том, как именно обрабатывать размеры страницы, и изменять её масштаб --&gt
...
`,
  ...
  [
    [`6. Разница между &ltscript&gt, &ltscript async&gt и &ltscript defer&gt?`],
    [
      `Когда браузер загружает HTML и доходит до тега &ltscript&gt...&lt/script&gt, он не может продолжать строить DOM. Он должен сначала выполнить скрипт. То же самое происходит и с внешними скриптами html&ltscript src=...&gt&lt/script&gt: браузер должен подождать, пока загрузится скрипт, выполнить его, и только затем обработать остальную страницу.
...
- Встроенные стили
  bash
     &ltp style=font-weight: bold; color: red;&gtОбратите внимание на этот текст.&lt/p&gt
  `,
    ],
  ],
  [
...
- rem – задаёт размер относительно шрифта &lthtml&gt, используется для удобства глобального масштабирования: элементы которые планируется масштабировать, задаются в rem, а JS меняет шрифт у &lthtml&gt.
...
| :invalid           | Соответствует элементу, например &ltinput&gt, в недопустимом состоянии.                            |
